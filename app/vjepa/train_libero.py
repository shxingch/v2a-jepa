...
def main(args, resume_preempt=False):
    ...
    # 获取动作预测头配置
    action_predictor_cfg = cfgs_model.get('action_predictor', {})

    # -- DATA
    cfgs_data = args.get('data')
    dataset_type = cfgs_data.get('dataset_type', 'videodataset')
    mask_type = cfgs_data.get('mask_type', 'multiblock3d')
    data_dir = cfgs_data.get('data_dir')
    batch_size = cfgs_data.get('batch_size')
    num_frames = cfgs_data.get('num_frames')
    frame_skip = cfgs_data.get('frame_skip', 4)
    tubelet_size = cfgs_data.get('tubelet_size', 2)
    view_type = cfgs_data.get('view_type', 'agentview')
    ...
    resize_to = cfgs_data_aug.get('resize_to', None)
    ...
    action_loss_weight = cfgs_loss.get('action_loss_weight', 0.5)
    ...
    # -- make csv_logger with additional action loss column
    ...
    if action_predictor_cfg.get('enabled', True):
        action_predictor = init_libero_model(
            encoder=encoder,
            action_predictor_cfg=action_predictor_cfg,
            device=device
        )
    ...
    # -- init optimizer and scheduler (包含动作预测器)
    optimizer_params = list(encoder.parameters()) + list(predictor.parameters())
    if action_predictor is not None:
        optimizer_params += list(action_predictor.parameters())
        
    optimizer, scaler, scheduler, wd_scheduler = init_opt(
        encoder=encoder,
        predictor=predictor,
        action_predictor=action_predictor,  # 添加动作预测器
        wd=wd,
        final_wd=final_wd,
        start_lr=start_lr,
        ref_lr=lr,
        ...
        betas=betas,
        eps=eps)
    # 只有在多GPU环境下才使用DistributedDataParallel
    if world_size > 1:
        encoder = DistributedDataParallel(encoder, static_graph=True)
        predictor = DistributedDataParallel(predictor, static_graph=True)
        target_encoder = DistributedDataParallel(target_encoder)
        if action_predictor is not None:
            action_predictor = DistributedDataParallel(action_predictor)
    ...
    def save_checkpoint(epoch, path):
        ...
        if action_predictor is not None:
            save_dict['action_predictor'] = action_predictor.state_dict()
        try:
    ...
    for epoch in range(start_epoch, num_epochs):
        ...
        action_loss_meter = AverageMeter()  # 添加动作损失记录
        ...
        for itr in range(ipe):
            ...
            def load_clips():
                ...
                # Libero数据集返回的是字典结构
                clips = udata['frames'].to(device, non_blocking=True)
                actions = udata['actions'].to(device, non_blocking=True)
                ee_states = udata['ee_states'].to(device, non_blocking=True)
                ...
                return (clips, actions, ee_states, _masks_enc, _masks_pred)
            clips, actions, ee_states, masks_enc, masks_pred = load_clips()
            ...
            def train_step():
                ...
                def action_loss_fn(pred_actions, true_actions):
                    """
                    计算动作预测损失
                    
                    Args:
                        pred_actions: 模型预测的动作，形状为 [B, 1568, 7]
                        true_actions: 真实动作，形状为 [B, 16, 7]
                    
                    Returns:
                        损失值
                    """
                    # 打印形状以便调试
                    # print(f"预测动作形状: {pred_actions.shape}, 真实动作形状: {true_actions.shape}")
                    
                    # 调整预测动作的形状以匹配真实动作
                    # 方法1: 如果预测动作是从特征图生成的，可以使用自适应池化将其调整为正确的大小
                    B, _, C = pred_actions.shape
                    _, T, _ = true_actions.shape
                    
                    # 使用自适应平均池化将预测动作的第二维从1568调整为16
                    pred_actions_resized = F.adaptive_avg_pool1d(pred_actions.transpose(1, 2), T).transpose(1, 2)
                    
                    # 确认形状匹配
                    # print(f"调整后的预测动作形状: {pred_actions_resized.shape}")
                    
                    # 计算损失
                    return F.mse_loss(pred_actions_resized, true_actions)

                def reg_fn(z):
                    return sum([torch.sqrt(zi.var(dim=1) + 0.0001) for zi in z]) / len(z)

                # Step 1. Forward
                loss_jepa, loss_action, loss_reg = 0., 0., 0.
                with torch.cuda.amp.autocast(dtype=dtype, enabled=mixed_precision):
                    ...
                    # 动作预测损失
                    if action_predictor is not None:
                        pred_actions = forward_action(clips)
                ...
                # 组合损失
                loss = loss_jepa + action_loss_weight * loss_action + reg_coeff * loss_reg

                # Step 2. Backward & step
                _enc_norm, _pred_norm, _action_norm = 0., 0., 0.
                if mixed_precision:
                ...
                if (epoch > warmup) and (clip_grad is not None):
                    ...
                    if action_predictor is not None:
                        _action_norm = torch.nn.utils.clip_grad_norm_(action_predictor.parameters(), clip_grad)
                if mixed_precision:
                ...
                grad_stats_action = None
                if action_predictor is not None:
                    grad_stats_action = grad_logger(action_predictor.named_parameters())
                    grad_stats_action.global_norm = float(_action_norm)
            ...
            (loss, loss_jepa, loss_action, loss_reg, _new_lr, _new_wd, 
             grad_stats, grad_stats_pred, grad_stats_action, optim_stats,), gpu_etime_ms = gpu_timer(train_step)
            ...
            if world_size > 1:
                ...
                input_var = float(clips.view(clips.shape[0], -1).var(dim=1).mean(dim=0))
                input_var_min = float(torch.min(clips.view(clips.shape[0], -1).var(dim=1)))
            input_var_meter.update(input_var)
            input_var_min_meter.update(input_var_min)
            jepa_loss_meter.update(loss_jepa)
            action_loss_meter.update(loss_action)
            ...
            # -- Logging
            def log_stats():
                action_grad_norm = grad_stats_action.global_norm if grad_stats_action is not None else 0.0
                csv_logger.log(
                    epoch + 1,
                    itr,
                    loss,
                ...
                if (itr % log_freq == 0) or np.isnan(loss) or np.isinf(loss):
                    ...
                    if grad_stats_action is not None:
                        logger.info(
                            '[%d, %5d] action_grad_stats: f/l[%.2e %.2e] mn/mx(%.2e, %.2e) %.2e'
                            % (epoch + 1, itr,
                               grad_stats_action.first_layer,
                               grad_stats_action.last_layer,
                               grad_stats_action.min,
                               grad_stats_action.max,
                               grad_stats_action.global_norm))
            log_stats()
